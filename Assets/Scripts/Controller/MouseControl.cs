...
public class MouseControl : MonoBehaviour
{
    ...
    private void OnMouseEnter()
    {
        ...
        if (lockPuzzle.transform.localScale.x != 0 && transform.position.x > -2.54f && transform.position.x < 2.33f)
            isShielded = true;
        else
            isShielded = false;
        if (!isShielded)
        {
            ...
            transform.localScale = new Vector3(initScale.x * 1.5f, initScale.y * 1.5f, initScale.z * 1.5f);
        }
    }
